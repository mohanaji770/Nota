...
export function HabitsScreen() {
  ...
  return (
    <main className="adaptive-tonal mx-auto min-h-[100dvh] w-full max-w-[430px] bg-surface px-5 pb-[calc(30px+var(--safe-bottom))] pt-[calc(10px+var(--safe-top))] dark:bg-surface-dark">
      <header className="sticky top-0 z-20 -mx-5 bg-surface/92 px-3 pt-[calc(6px+var(--safe-top))] backdrop-blur-xl dark:bg-surface-dark/92">
        <div className="flex min-h-14 items-center gap-2">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-[1.55rem] font-bold leading-8 text-white/88">العادات</h1>
            <p className="text-[0.72rem] font-medium leading-4 text-white/32">تابع آخر 7 أيام بهدوء</p>
          </div>
        </div>
      </header>
      ...
      <section className="mt-4 rounded-2xl bg-white/[0.06] p-4 ring-1 ring-white/[0.07]">
        {notificationPermission !== "granted" ? (
          <button
            ...
            className="mb-3 flex min-h-11 w-full items-center gap-2 rounded-xl bg-accent/15 px-3 text-right text-[0.74rem] font-bold leading-5 text-accent ring-1 ring-accent/25 transition active:scale-[0.98] disabled:opacity-50"
          >
            <Bell size={16} />
        ...
        <label className="block">
          <span className="mb-1 block text-[0.68rem] font-bold text-white/38">عنوان العادة</span>
          <input
            value={title}
            onChange={(event) => setTitle(event.target.value)}
            placeholder="مثلاً: قراءة، مشي، شرب ماء"
            className="h-11 w-full rounded-xl bg-white/[0.06] px-3 text-[0.82rem] text-white/76 outline-none ring-1 ring-white/[0.08] placeholder:text-white/22"
          />
        </label>
        <div className="mt-3 flex items-end gap-2">
          <label className="min-w-0 flex-1">
            <span className="mb-1 block text-[0.68rem] font-bold text-white/38">تذكير يومي</span>
            <input
              type="time"
              value={reminderTime}
              onChange={(event) => setReminderTime(event.target.value)}
              className="h-11 w-full rounded-xl bg-white/[0.06] px-3 text-[0.82rem] text-white/76 outline-none ring-1 ring-white/[0.08]"
            />
          </label>
          <button
            type="button"
            onClick={handleCreate}
            className="grid h-11 w-12 shrink-0 place-items-center rounded-xl bg-accent text-white transition active:scale-95"
      ...
      <section className="mt-5 space-y-3" aria-label="قائمة العادات">
        {habits.length === 0 ? (
          <div className="py-14 text-center">
            ...
            <h2 className="mt-5 text-[1.1rem] font-bold text-white/82">ابدأ عادة واحدة</h2>
            <p className="mx-auto mt-2 max-w-[270px] text-[0.78rem] font-medium leading-6 text-white/32">
              ...
              <div className="flex items-start gap-3">
                <div className="min-w-0 flex-1">
                  <input
                    ...
                    className="w-full bg-transparent text-[0.95rem] font-bold leading-6 text-white/82 outline-none"
                    aria-label="عنوان العادة"
                  />
                  <div className="mt-1 flex w-fit items-center gap-1.5 text-[0.66rem] font-medium text-white/32">
                    <Bell size={11} />
                    <input
                      type="time"
                ...
                <button
                  ...
                  className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-white/[0.06] text-white/35 transition active:scale-95 hover:bg-destructive/10 hover:text-destructive"
                      ...
                      className={cn(
                        "flex flex-col items-center justify-center rounded-2xl py-2.5 text-center transition active:scale-[0.96]",
                          ...
                          : day.isToday
                            ...
                            : "bg-white/[0.04] text-white/28 hover:bg-white/[0.08]"
                      )}
                      aria-label={`${habit.title} ${day.key}`}
                    >
                      <span className="text-[0.55rem] font-bold uppercase leading-3 opacity-65">{day.day}</span>
      ...
      {deletedHabit ? (
        <div className="fixed inset-x-4 bottom-[calc(18px+var(--safe-bottom))] z-50 mx-auto flex max-w-[380px] items-center gap-3 rounded-2xl bg-[#1c1c1e] px-4 py-3 text-[0.78rem] font-semibold text-white shadow-2xl ring-1 ring-white/[0.08]">
          <p className="min-w-0 flex-1">تم حذف العادة</p>
          <button
            type="button"
            onClick={handleUndoDelete}
            className="rounded-full bg-white/[0.08] px-3 py-2 text-[0.72rem] font-bold text-white transition active:scale-95"
          >
            تراجع
          </button>
        </div>
