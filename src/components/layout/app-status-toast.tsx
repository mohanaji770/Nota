...
export function AppStatusToast() {
  ...
  return (
    <div className="fixed inset-x-4 bottom-[calc(18px+var(--safe-bottom))] z-[70] mx-auto flex max-w-[380px] items-center gap-3 rounded-[22px] bg-[#202020] px-4 py-3 text-right text-[0.78rem] font-semibold leading-5 text-[#f7f7f2] shadow-2xl ring-1 ring-white/[0.08]">
      <p className="min-w-0 flex-1">{message}</p>
      <button
        type="button"
        onClick={clearOperationError}
        className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/[0.08] text-white/70 transition active:scale-95"
        aria-label="إغلاق"
      >
        <X size={15} />
      </button>
